...
function getUUID(coll) {
    return coll.getDB().getCollectionInfos({name: collName})[0].info.uuid;
}

function getShardKeyMinRanges(coll) {
    let a = coll.getDB()
                .getSiblingDB("config")
                .chunks.find({uuid: getUUID(coll)}, {min: 1, _id: 0})
                .sort({min: 1});
    return a.toArray();
}
...
const wcCommandsTests = {
    ...
    _getAuditConfigGeneration: {skip: "internal command"},
    ...
    abortShardSplit: {skip: "unused command"},
    abortTransaction: {
        success: {
    ...
    commitShardSplit: {skip: "unused command"},
    commitTransaction: {
        noop: {
    ...
    donorAbortMigration: {skip: "does not accept write concern"},
    donorForgetMigration: {skip: "does not accept write concern"},
    donorStartMigration: {skip: "does not accept write concern"},
    drop: {
        noop: {
    ...
    forgetShardSplit: {skip: "unused command"},
        ...
        success: {
            ...
            setupFunc: (coll, cluster, clusterType, secondariesRunning, optionalArgs) => {
                ...
                optionalArgs.originalShard =
                    coll.getDB()
                        .getSiblingDB("config")
                        .chunks.findOne({uuid: getUUID(coll), min: keyToMove})
                        .shard;
                ...
                // Make sure we set the fp on the owning shard
                assert.commandWorked(coll.insert(keyToMove));
                let fpConn = coll.getDB();
                let owningShard = cluster.getShard(coll, keyToMove, false /* includeEmpty */);
                fpConn = owningShard.getDB(dbName);
                optionalArgs.failpoint =
                    configureFailPoint(fpConn,
                                       "overrideDefaultShardingCatalogClientWriteConcernTimeout",
                                       {"wTimeoutMillis": 1000});
            ...
            confirmFunc: (res, coll, cluster, clusterType, secondariesRunning, optionalArgs) => {
                ...
                assert.eq(coll.getDB()
                              .getSiblingDB("config")
                              .chunks.findOne({uuid: getUUID(coll), min: keyMoved})
                              .shard,
...
const wcTimeseriesViewsCommandsTests = {
    ...
    abortMoveCollection: {skip: "does not accept write concern"},
    abortReshardCollection: {skip: "does not accept write concern"},
    abortShardSplit: {skip: "does not accept write concern"},
        ...
        success: {
            ...
            confirmFunc: (res, coll, cluster, clusterType) => {
                ...
                if (clusterType != "sharded" || !writeWithoutSkOrId) {
                    ...
                    // The two phase write path returns WriteConcernFailed in the write
                    // errors array.
                    assert.commandWorkedIgnoringWriteErrorsAndWriteConcernErrors(res);
                    assert(res.writeErrors && res.writeErrors.length == 3);
                    res.writeErrors.forEach((err) => {
                        assert.eq(err.code, ErrorCodes.WriteConcernFailed);
                    });
                    assert.eq(res.nModified, 0);
                    assert.eq(coll.find({"meta.y": 1}).toArray().length, 3);
                }
        ...
        failure: {
            ...
            confirmFunc: (res, coll, cluster, clusterType) => {
                ...
                if (clusterType != "sharded" || !writeWithoutSkOrId) {
                    ...
                    assert.commandWorkedIgnoringWriteErrorsAndWriteConcernErrors(res);
                    assert(res.writeErrors && res.writeErrors.length == 3);
                    assert.eq(res.writeErrors[0].code, ErrorCodes.WriteConcernFailed);
                    assert.includes([ErrorCodes.BadValue, ErrorCodes.InvalidOptions],
                                    res.writeErrors[1].code);
                    assert.eq(res.writeErrors[2].code, ErrorCodes.WriteConcernFailed);
        ...
        noop: {
            ...
            confirmFunc: (res, coll, cluster, clusterType) => {
                ...
                if (clusterType != "sharded" || !writeWithoutSkOrId) {
                    ...
                    assert.eq(res.cursor.firstBatch[1].code, ErrorCodes.WriteConcernFailed);
                    assert.eq(res.nErrors, 1);
                }
        ...
        failure: {
            ...
            confirmFunc: (res, coll, cluster, clusterType) => {
                ...
                if (clusterType != "sharded" || !writeWithoutSkOrId) {
                    ...
                    assert.eq(res.cursor.firstBatch[2].code, ErrorCodes.WriteConcernFailed);
        ...
        noop: {
            ...
            confirmFunc: (res, coll, cluster, clusterType) => {
                ...
                if (clusterType != "sharded" || !writeWithoutSkOrId) {
                    ...
                    // write with WriteConcernFailed, so the insert will not execute.
                    assert.eq(res.cursor.firstBatch.length, 2);
    ...
    try {
        ...
        try {
            ...
            if (res.raw) {
                ...
                // Some commands fail the command with WriteConcernFailed as a top-level error,
...
function shouldSkipTestCase(clusterType, command, testCase, shardedCollection, writeWithoutSk) {
    // Cannot downgrade to FCV 7.0 when using a config shard because this feature does not exist on
    // v7.0.
    if (TestData.configShard && command == "setFeatureCompatibilityVersion") {
        return true;
    }
    ...
    if (testCase == "noop") {
        ...
        if (clusterType == "sharded" &&
            (shardedDDLCommandsRequiringMajorityCommit.includes(command) ||
             ...
             command == "enableSharding" || command == "dropAllRolesFromDatabase")) {
...
export function checkWriteConcernBehaviorForAllCommands(
    ...
    // Randomly execute commands with 10% probability to reduce test runtime.
    ...
    const selectedCommands = commandsList.filter(() => Math.random() < 0.1);
