...
(function() {
    ...
    const allCommands = {
        ...
        _cloneCatalogData: {skip: isPrimaryOnly},
        ...
        _movePrimary: {skip: isPrimaryOnly},
        ...
        geoNear: {
            command: {geoNear: collName, near: [-42, 42]},
            expectFailure: true,
            expectedErrorCode: ErrorCodes.NotMasterOrSecondary,
        },
        geoSearch: {
        ...
        restartCatalog: {skip: isNotAUserDataRead},
    ...
    assert.commandWorked(primaryDb.getCollection(collName).insert({a: 42, loc: [1, 1]}));
    assert.commandWorked(primaryDb.getCollection(collName).runCommand(
