...
__checkpoint_name_check(WT_SESSION_IMPL *session, const char *uri)
{
     ...
     * file objects, and not on LSM trees or Helium devices. If a target list is configured for the
     * checkpoint, this function is called with each target list entry; check the entry to make sure
     * it's backed by a file. If no target list is configured, confirm the metadata file contains no
     * non-file objects. Skip any internal system objects. We don't want spurious error messages,
     * other code will skip over them and the user has no control over their existence.
     */
    if (uri == NULL) {
  ...
  WT_SESSION_IMPL *session, const char *cfg[], bool *fullp, bool *use_timestampp, bool *can_skipp)
{
    ...
    *use_timestampp = use_timestamp = cval.val != 0;
     ...
     */
    if (use_timestamp && txn_global->has_stable_timestamp && txn_global->last_ckpt_timestamp != 0 &&
  ...
  WT_SESSION_IMPL *session, bool is_checkpoint, bool force, bool need_tracking, const char *cfg[])
{
    ...
    /*
     * We are now done with the local use of the name. Free the local allocation, if needed.
     */
...
#define WT_MAX_CHECKPOINT_DROP 4
    ...
    /*
     * There are special tree: those being bulk-loaded, salvaged, upgraded or verified during the
     ...
     * ordinary checkpoints will skip files that cannot be opened normally.
     */
...
__checkpoint_tree(WT_SESSION_IMPL *session, bool is_checkpoint, const char *cfg[])
{
    ...
    /*
     * All blocks being written have been written; set the object's write generation.
     */
    WT_CKPT_FOREACH (ckptbase, ckpt)
...
__wt_checkpoint_close(WT_SESSION_IMPL *session, bool final)
{
    ...
    if (!btree->modified && !bulk) {
        ...
        return (__wt_txn_visible_all(session, btree->rec_max_txn, btree->rec_max_timestamp) ?
            __wt_cache_op(session, WT_SYNC_DISCARD) :
            EBUSY);
    }
