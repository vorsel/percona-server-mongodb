...
retry:
    while (slot < max_entries) {
        ...
        if (dhandle == NULL) {
             ...
             * have a saved handle, start from the beginning of the list.
             */
    ...
    /*
     * Walk the list of files a few times if we don't find enough pages. Try two passes through all
     * the files, give up when we have some candidates and we aren't finding more.
     */
    if (slot < max_entries &&
      (retries < 2 ||
