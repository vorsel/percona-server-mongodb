...
TEST(ModuleLoaderTest, ModulesNotSupportedInServerEnvironment) {
    mongo::ScriptEngine::setup(ExecutionEnvironment::Server);
    {
        ...
        // Module loading is not supported in the server execution environment: the module
        // loader is never constructed (so getModuleLoader() must not be called) and no
        // filesystem-backed module hooks are registered. This is what prevents server-side
        // JavaScript from reading host files via import().
...
// These negative cases trip a tassert. Caught tasserts leave the tripwire counter set, which
// aborts the binary at process exit, so they are written as DEATH_TESTs (MongoDB's convention
// for testing tasserts) rather than ASSERT_THROWS_CODE catches.
...
DEATH_TEST(ModuleLoaderDeathTest, ConstructInServerEnvironment, "12883202") {
    // The ModuleLoader constructor itself refuses the server execution environment, as a
    // backstop against any caller that bypasses MozJSImplScope's gating.
...
TEST(ModuleLoaderTest, ImportDisabledInServerEnvironment) {
    ...
    // Write a real file on disk that a malicious import would try to read (e.g. a keyFile or
    // PEM).
    ...
    // On the server, module loading is not supported: a failed classic-script compilation is
    // never retried as a module, so the import is never resolved and the file is never read.
    // The original SyntaxError propagates instead. The scope itself was constructed
    // successfully above, proving the self-contained server setup scripts load without the
    // module loader.
