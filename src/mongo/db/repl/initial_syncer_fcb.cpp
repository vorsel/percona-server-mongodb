...
void InitialSyncerFCB::_transferFileCallback(
    ...
    status = syncSourceConn.connect(_syncSource, "File copy-based initial sync", boost::none);
    if (status.isOK()) {
    ...
    if (!cloneStatus.isOK()) {
        ...
        if (nextFileIdx < _remoteFiles.size()) {
            ...
            executor::RemoteCommandRequest replSetGetStatusRequest(
                _syncSource, "admin", BSON("replSetGetStatus" << 1), nullptr);
            auto scheduleResult =
                (*_attemptExec)
                    ->scheduleRemoteCommand(
...
Status resetLocalLastVoteDocument(OperationContext* opCtx) try {
    writeConflictRetry(
        opCtx, "reset last vote document", NamespaceString::kLastVoteNamespace.toString(), [opCtx] {
    ...
    LOGV2_DEBUG(
        128472, 1, "Exception during reset last vote document", "error"_attr = e.toString());
    return e.toStatus();
}
