...
public:
    ...
    public:
         ...
         */
        boost::optional<repl::OplogEntry> checkStatementExecutedAndFetchOplogEntry(
    ...
    private:
        ...
        boost::optional<Timestamp> _getCommitTimestamp() const;

        // Checks whether the given statementId for the specified transaction has already executed
        // in any external or internal sessions associated with this session (see the header comment
        // for RetryableWriteTransactionParticipantCatalog for the definitions of these sessions).
        // - If the statement has been executed, returns the opTime for the oplog entry that was
        //   generated by that write and the commitTimestamp for the write if it was executed in
        //   a multi-document transaction.
        // - Otherwise, returns boost::none.
        //
        // Throws if the session has been invalidated or the active transaction number doesn't
        // match.
        boost::optional<StatementInfo> _checkStatementExecuted(OperationContext* opCtx,
                                                               StmtId stmtId) const;

        // Same as above, but only checks if the specified transaction has already executed in this
        // sessions.
        boost::optional<StatementInfo> _checkStatementExecutedSelf(StmtId stmtId) const;
...
private:
    ...
    struct ObservableState {
        ...
        // Track the commit timestamp if this is a committed transaction.
        Timestamp commitTimestamp;
