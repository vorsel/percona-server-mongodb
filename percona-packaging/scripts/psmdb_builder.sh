...
#
      ...
      if [ x"${DEBIAN}" != "xbullseye" -a x"${DEBIAN}" != "xjammy" -a x"${DEBIAN}" != "xnoble" ]; then
...
build_source_deb(){
    ...
    #pip install pyyaml==5.3.1 --no-build-isolation
...
build_tarball(){
    ...
    #
    #
    PSM_TARGETS="mongod mongos mongobridge perconadecrypt $SPECIAL_TAR"
    ...
    if [ ${DEBUG} = 0 ]; then
        buildscripts/scons.py CC=${CC} CXX=${CXX} --disable-warnings-as-errors --release --ssl --opt=on -j${NCPU} --use-sasl-client --wiredtiger CPPPATH="${INSTALLDIR}/include ${AWS_LIBS}/include" LIBPATH="${INSTALLDIR}/lib ${AWS_LIBS}/lib ${AWS_LIBS}/lib64" LINKFLAGS="${LINKFLAGS}" ${PSM_REAL_TARGETS[@]} || exit $?
    else
        buildscripts/scons.py CC=${CC} CXX=${CXX} --disable-warnings-as-errors --ssl --dbg=on -j${NCPU} --use-sasl-client \
        CPPPATH="${INSTALLDIR}/include ${AWS_LIBS}/include" LIBPATH="${INSTALLDIR}/lib ${AWS_LIBS}/lib ${AWS_LIBS}/lib64" LINKFLAGS="${LINKFLAGS}" --wiredtiger ${PSM_REAL_TARGETS[@]} || exit $?
    fi
    #
