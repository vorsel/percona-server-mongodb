...
get_sources(){
    ...
    echo "PRODUCT_FULL=${PRODUCT_FULL}" >> ${WORKDIR}/percona-server-mongodb-36.properties
    echo "VERSION=${PSM_VER}" >> ${WORKDIR}/percona-server-mongodb-36.properties
    echo "RELEASE=${PSM_RELEASE}" >> ${WORKDIR}/percona-server-mongodb-36.properties
    echo "MONGO_TOOLS_TAG=${MONGO_TOOLS_TAG}" >> ${WORKDIR}/percona-server-mongodb-36.properties
...
install_golang() {
    ...
    rm -rf /usr/local/go1.8 /usr/local/go1.9 /usr/local/go1.9.2 /usr/local/go1.11 /usr/local/go
    mv go1.11 /usr/local/
...
build_tarball(){
    ...
    LIBLIST="libcrypto.so libssl.so libsasl2.so libpcap.so libcurl.so libldap liblber libgssapi_krb5.so libkrb5.so libkrb5support.so libk5crypto.so libfreebl3.so libssl3.so libsmime3.so libnss3.so libnssutil3.so libplds4.so libplc4.so libnspr4.so libssl3.so libplds4.so libidn.so"
